...
function PackagePage() {
  ...
  const dep = (packageData as any).deposit || "500";
  const paypalEmail = (packageData as any).paypalEmail || "jumaadventuresandsafaris@gmail.com";
  const paybillNo = (packageData as any).paybillNo || "PENDING";
  const paybillAcc = (packageData as any).paybillAcc || "MARAMARA";
  ...
  // --- NEW DESIGN ONLY FOR MASAI MARA ---
  if (isMara) {
    return (
      <div className="min-h-screen bg-white pb-24">
        {/* PHOTOS PLAYING UNDERGROUND */}
          ...
          <div className="absolute bottom-0 left-0 right-0 p-4">
            <div className="overflow-hidden bg-[#F5B400] rounded-full py-1">
              <p className="animate-marquee whitespace-nowrap font-black text-black text-[11px] tracking-[0.2em]">
                {packageData.title.toUpperCase()} • BIG FIVE • MIGRATION • BALLOON • SUNSET • {packageData.highlights.join(" • ").toUpperCase()} •
              </p>
            </div>
        ...
        {/* PAY NOW POPUP - MPESA + PAYPAL */}
        {showPay && (
          <div className="fixed inset-0 bg-black/70 z-50 flex items-end justify-center">
            <div className="bg-white w-full max-w-md rounded-t-[24px] p-5 animate-slide">
              ...
              <p className="text-xs text-gray-500">Total ${full} • Secure slot with ${dep} deposit</p>

              <a href={paypalDep} target="_blank" className="block mt-4 bg-[#F5B400] text-black text-center py-4 rounded-full font-black">Secure with ${dep} Deposit via PayPal</a>

              <div className="grid grid-cols-2 gap-3 mt-3">
                <a href={paypalFull} target="_blank" className="bg-black text-white text-center py-3 rounded-full font-bold text-sm">PayPal Full ${full}</a>
              ...
              <div className="bg-gray-100 p-3 rounded-xl mt-4 text-[11px]">
                <p className="font-bold">M-Pesa Paybill (Money goes direct):</p>
                <p>Paybill: <b>{paybillNo}</b> • Acc: <b>{paybillAcc}</b> • Amount: ${dep} or ${full}</p>
                <p className="mt-1 text-gray-500">PayPal: {paypalEmail}</p>
              </div>
            </div>
          </div>
        )}
  ...
  // --- OLD DESIGN FOR ALL OTHER PACKAGES (unchanged) ---
  return (
    <div className="max-w-6xl mx-auto p-4 pb-24 bg-white">
      ...
      <div className="mb-6"><h2 className="text-2xl font-bold mb-3">Itinerary</h2>{(packageData.itinerary?? []).map((day: any) => (<div key={day.dayNum} className="border p-4 rounded-lg mb-3"><h3 className="font-bold">Day {day.dayNum}: {day.title}</h3><ul className="list-disc ml-5 mt-2 text-gray-700">{(day.details?? []).map((d: string, i: number) => <li key={i}>{d}</li>)}</ul></div>))}</div>
      <div className="grid md:grid-cols-2 gap-6 mb-6"><div><h3 className="font-bold mb-2 text-green-700">What's Included</h3><ul className="space-y-1">{(packageData.includes?? []).map((i: string, idx: number) => <li key={idx}>✓ {i}</li>)}</ul></div><div><h3 className="font-bold mb-2 text-red-700">What's Excluded</h3><ul className="space-y-1">{(packageData.excludes?? []).map((e: string, idx: number) => <li key={idx}>✗ {e}</li>)}</ul></div></div>
