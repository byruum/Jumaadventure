...
function PackagePage() {
  ...
  useEffect(() => {
    ...
    const t1 = globalThis.setInterval(() => setCurrent((p: number) => (p + 1) % validGallery.length), 4000);
    const t2 = globalThis.setInterval(() => setTextIndex((p: number) => (p + 1) % 3), 2500);
      ...
      <div className="bg-[#FAF7F2] min-h-screen pb-28">
        <div className="relative h-[88vh] bg-black overflow-hidden">
          {validGallery.map((img: string, idx: number) => (
            <img key={idx} src={img} alt="" onError={(e) => { (e.currentTarget as HTMLImageElement).src = fallbackMara[0] }} className={`absolute inset-0 w-full h-full object-cover transition-all duration-[3000ms] ${idx===current?'opacity-100':'opacity-0'}`} />
          ))}
          <div className="absolute inset-0 bg-black/60" />
          <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-6">
            <p className="text-[#F5B400] font-black tracking-[0.35em] text-[11px] md:text-[13px]">{["MASAI MARA NATIONAL RESERVE","BIG FIVE • GREAT MIGRATION","ADVENTURE OF A LIFETIME"][textIndex]}</p>
        ...
        <div className="max-w-6xl mx-auto px-4 mt-8">
          <div className="bg-white rounded-[20px] p-8 border">
            <h2 className="font-black text-xl">The Journey</h2>
            <p className="text-[14px] text-gray-600 mt-3">{packageData.journey}</p>
          </div>
          <div className="bg-white rounded-[20px] p-8 border mt-6">
            ...
            <div className="mt-6 space-y-4">
              {packageData.itinerary.map((day: Day) => (
                <div key={day.dayNum} className="bg-[#FAF7F2] rounded-xl p-4 border"><h3 className="font-black text-[14px]">Day {day.dayNum}: {day.title}</h3><ul className="mt-2">{day.details.map((x: string, i: number)=><li key={i} className="text-[13px]">• {x}</li>)}</ul></div>
              ))}
            </div>
          </div>
        </div>
          ...
          <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
            <div className="bg-white w-full max-w-[420px] rounded-[24px] p-6">
              ...
              <div className="grid grid-cols-2 gap-3 mt-3">
                <a href={paypalFull} target="_blank" rel="noreferrer" className="bg-black text-white text-center py-3.5 rounded-full font-bold">PayPal Full ${full}</a>
                <div className="bg-[#0B6A2B] text-white text-center py-3.5 rounded-full font-bold text-[11px]">M-Pesa Paybill {paybillNo} Acc {paybillAcc}</div>
              </div>
            </div>
          </div>
        )}
